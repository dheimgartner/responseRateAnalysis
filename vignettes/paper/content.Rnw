...
fit <- lm(y ~ 0 + x + yes_no + no_no + no_yes,  # yes_yes is the reference
...
fit <- lm(y ~ 0 + x + yes_no + no_no + no_yes,
  ...
  m_list_ <-
    map(m_list, function(x) {
      if (length(x$coefficients) == 2) {
        ...
        names(x$coefficients) <- c("Response burden", "Yes, no", "No, no", "No, yes")
      }
      x
    })
  return(m_list_)
...
geom_response <- function(mapping = NULL, data = NULL, col, shade = "grey", lw = 1.5, alpha = 0.2) {
  list(
...
plot_by_category <- function(df1, df2, survey_data, cat, col, shape, ylim = c(0, 100)) {
  ...
  ggplot() +
    ...
    geom_response(data = e_yy_l, col = "deeppink") +
...
fit_t <- fit_t <- lm(y ~ 0 + x + yes_no + no_no + no_yes + year,
                     data = dat, weights = weight)
  ...
  m_list_ <-
    map(m_list, function(x) {
      if (length(x$coefficients) == 3) {
        ...
        names(x$coefficients) <- c("Response burden", "Yes, no", "No, no", "No, yes", "Time-trend")
      }
      x
    })
  return(m_list_)
